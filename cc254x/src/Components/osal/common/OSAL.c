...
uint8 osal_init_system( void )
{
  ...
  osalInitTasks();//³õÊ¼»¯ÏµÍ³ÈÎÎñ£¬ ÕâÒ»¸öÈÎÎñ³õÊ¼»¨·Ç³£¹Ø¼ü
...
void osal_run_system( void )
{
  ...
  Hal_ProcessPoll();    //²éÑ¯Êý¾Ý£¬±ÈÈç´®¿ÚÊý¾Ý£¬usbÊý¾ÝµÈ

  do { // Ñµ»·Ñ°ÕÒÊÇ·ñÓÐÊÂ¼þ£¬ÓÐÊÂ¼þµÄ»°£¬¾ÍÁ¢ÂíÍË³ö£¬appÓ¦ÓÃÓÅÏÈ¼¶×îµÍ
  ...
  if (idx < tasksCnt)// ÕÒµ½ÁËÊÂ¼þ
  {
    uint16 events;
    halIntState_t intState;

    HAL_ENTER_CRITICAL_SECTION(intState);  // ¹Ø±ÕÖÐ¶Ï
    events = tasksEvents[idx];        //¶ÁÈ¡¸ÃÈÎÎñµÄÊÂ¼þ(ÊÂ¼þ¿ÉÄÜ²»Ö¹1¸ö)
    tasksEvents[idx] = 0; // Çå³ýÊ±¼ä¼ÇÂ¼£¬ÔÚÖ´ÐÐÈÎÎñ´¦Àíº¯ÊýÆÚ¼äÓÐ¿ÉÖÃÉÏÐÂÊÂ¼þ
    HAL_EXIT_CRITICAL_SECTION(intState);   // ¿ªÆôÖÐ¶Ï

    activeTaskID = idx;
    events = (tasksArr[idx])( idx, events );// tasksArr ÊÇÈÎÎñ´¦Àíº¯ÊýÖ¸Õë
